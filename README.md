...
## Training
Train a semantic LoRA using the Python script:

`python train_image_script.py --c=train_image`

This command will use `train_image.yaml` from the `config` folder for training. The trained semantic LoRA will then be saved to the `output_folder`. For example:
...
This script will generate images based on `train_image.yaml`.

## Real Image Editing
### Config file
The config file for real image editing (e.g., `train_image_real.yaml`) contains additional settings:
