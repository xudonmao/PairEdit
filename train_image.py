...
for epoch in range(max_train_steps):
    ...
    logs = {"loss2": loss2.item(), "loss1": loss1.item(),"lr": lr_scheduler.get_last_lr()[0]}
...
print("Saving...")
network_content.save_weights(
    f"{save_path}/{dataset}_{cfg_eta}eta_{noise_scale}ns_{num_inference_steps}inf_{epoch+1}epoch_0.pt",
    dtype=weight_dtype,
)
network_semantic.save_weights(
    f"{save_path}/{dataset}_{cfg_eta}eta_{noise_scale}ns_{num_inference_steps}inf_{epoch+1}epoch.pt",
    dtype=weight_dtype,
)
flush()
print("Done.")