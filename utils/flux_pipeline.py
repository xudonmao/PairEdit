...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    @torch.no_grad()
    def get_noisy_image(self, image, step, generator):
        vae = self.vae
    ...
    @torch.no_grad()
    def add_noise_precision(self, image, step, generator):
        vae = self.vae
        ...
        init_latents = init_latents.to(device=device, dtype=vae.dtype)

        shape = init_latents.shape

        noise = torch.randn(shape, generator=generator).to(device=device, dtype=vae.dtype)
    ...
    @torch.no_grad()
    def add_noise_rect(self, image, step, generator):
        vae = self.vae
        ...
        # add noise
        init_latents_add_noise = (1.0 - sigmas[step]) * init_latents + sigmas[step] * noise
        # init_latents_add_noise = self.scheduler.scale_noise(init_latents, torch.tensor([self.scheduler.timesteps[step]], device=device), noise)
